...
Let's practice working with the codesandbox editors.

With just a few steps we can dynamically display the current time on our page:
...
We're going to gradually build up a template for you to practice using Three JS.

 - Please fork a new second copy of the <a href="https://codesandbox.io/embed/cpc-w02-t01starter-djsj88?fontsize=14&hidenavigation=1&theme=dark">original starting point sandbox from above</a>
 - View your `dashboard` on codesandbox to see you now have two sandboxes. Go and give that a try using your new account. 😀
 - Rename the most recent sandbox "CPC TEMPLATE"
 - We'll use this one to gradually build up a template for you to practice using Three JS

Now that you've got a second sandbox ready to go...

 - Repeat task 1 now and add our overlay and buttons in `index.html`
 - Try editing `styles.css` to change the appearance of your play button (which should have id `startButton`)
...
Expand the sections below to see how the `index.html` `styles.css` and `index.js` files should look. 
